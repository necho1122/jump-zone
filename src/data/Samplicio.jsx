	...
	render() {
		...
		return (
			<div className='samplicio-container'>
				...
				<button className='add-host-button' onClick={() => {this.handleClassChange(); this.handleSearchClassChange()}}>
          Add host
        </button>
					<input
          className='search-input'
					...
					{search && filteredNotes.length > 0 ? ( // Cambia esto
						filteredNotes.map((note) => (
							<div key={note.id} className='search-results'>
								<h4>{note.host}</h4>
								<p><b>Link: </b>{note.link}</p>
								<p>{note.obs}</p>
							</div>
						))
					) : (
            ...
            className='obs-input add-input'
					...
					<button
						...
						onClick={() => {this.addNote(); this.setState({searchResults: 'search-section'}); this.setState({addHostSection: 'add-host'})}}
					>
						Add
					</button>
				</div>
